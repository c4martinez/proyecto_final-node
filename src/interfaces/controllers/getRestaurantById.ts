...
export async function getRestaurantById(req: Request, res: Response) {
  ...
  try {
    ...
    if (!restaurant) {
      // Si no se encuentra un restaurante con el ID especificado, se retorna un mensaje de error en el código de estado 404 (No encontrado)
    ...
    return res
      .status(500)
      .json({ message: 'Error fetching restaurant by ID.' });
  }
}
